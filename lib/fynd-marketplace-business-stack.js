...
class FyndMarketplaceBusinessStack extends Stack {
  constructor(scope, id, props) {
    ...
    const fyndOrderReturnDLQ = new sqs.Queue(
      this,
      `${resourcePrefix}FYND_ORDER_RETURN_DLQ`,
      {
        queueName: `${resourcePrefix}FYND_ORDER_RETURN_DLQ`,
        visibilityTimeout: cdk.Duration.seconds(300),
      }
    );
    ...
    const fyndOrderReturnSQS = new sqs.Queue(
      this,
      `${resourcePrefix}FYND_ORDER_RETURN_Q`,
      {
        queueName: `${resourcePrefix}FYND_ORDER_RETURN_Q`,
        visibilityTimeout: cdk.Duration.minutes(45),
        deadLetterQueue: {
          maxReceiveCount: 3,
          queue: fyndOrderReturnDLQ,
        },
      }
    );
    ...
    const fyndOrderReturnWebhookLambda = new lambda.Function(
      this,
      `${props.resourcePrefix}fyndOrderReturnWebhookLambda`,
      {
        runtime: props.runtime,
        layers: allLayersList,
        role: lambdaRole,
        functionName: `${props.resourcePrefix}fyndOrderReturnWebhookLambda`,
        code: lambda.Code.asset("lambdas/fyndWebhook/order-return"),
        handler: "fyndOrderReturnWebhookLambda.handler",
        environment: {
          fyndOrderReturnSQS: `${fyndOrderReturnSQS.queueUrl}`,
          REGION: props.env.region,
        },
      }
    );
    ...
    const fyndOMSOrderExportLambda = new lambda.Function(
      ...
      {
        description: "Lambda to export fynd OMS orders",
        runtime: props.runtime,
        layers: allLayersList,
        role: lambdaRole,
        ...
        memorySize: 256,
        environment: {
          CONTAINER_DETAILS_TABLE: `${resourcePrefix}${dbTables.ContainerDetails}`,
          REGION: props.env.region,
    ...
    const fyndOMSOrderReturnLambda = new lambda.Function(
      this,
      `${props.resourcePrefix}fyndOMSOrderReturnLambda`,
      {
        description: "Lambda to export fynd OMS Return Order",
        runtime: props.runtime,
        layers: allLayersList,
        role: lambdaRole,
        functionName: `${props.resourcePrefix}fyndOMSOrderReturnLambda`,
        code: lambda.Code.asset("lambdas/fyndOMSOrderReturn"),
        handler: "fyndOMSOrderReturnLambda.handler",
        timeout: cdk.Duration.minutes(15),
        logRetention: logs.RetentionDays.ONE_MONTH,
        reservedConcurrentExecutions: 1,
        memorySize: 256,
    ...
    api.root
      .resourceForPath("fynd/order-return")
      .addMethod(
        "POST",
        new apigw.LambdaIntegration(fyndOrderReturnWebhookLambda)
      );
  }
}
