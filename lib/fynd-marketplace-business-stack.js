...
class FyndMarketplaceBusinessStack extends Stack {
  constructor(scope, id, props) {
    ...
    const niceOmsOrderUpdatesDLQ = new sqs.Queue(
      this,
      `${resourcePrefix}NICE_OMS_ORDER_UPDATES_DLQ`,
      {
        queueName: `${resourcePrefix}NICE_OMS_ORDER_UPDATES_DLQ`,
        visibilityTimeout: cdk.Duration.seconds(300),
      }
    );

    const niceOmsOrderUpdatesSQS = new sqs.Queue(
      this,
      `${resourcePrefix}NICE_OMS_ORDER_UPDATES_Q`,
      {
        queueName: `${resourcePrefix}NICE_OMS_ORDER_UPDATES_Q`,
        visibilityTimeout: cdk.Duration.minutes(45),
        deadLetterQueue: {
          maxReceiveCount: 3,
          queue: niceOmsOrderUpdatesDLQ,
        },
      }
    );
    ...
    // Add S3 event notification to trigger the Lambda function when an object is created
    bucket.addEventNotification(
      s3.EventType.OBJECT_CREATED,
      new s3Notifications.SqsDestination(niceOmsOrderUpdatesSQS), {
        prefix: 'nice-oms/updates',
        suffix: '.xml'
      },
    );
    ...
    const niceOmsOrderUpdatesLambda = new lambda.Function(
      this,
      `${props.resourcePrefix}niceOmsOrderUpdatesLambda`,
      {
        description: "Lambda to process Nice OMS Order Updates",
        runtime: props.runtime,
        layers: allLayersList,
        role: lambdaRole,
        functionName: `${props.resourcePrefix}niceOmsOrderUpdatesLambda`,
        code: lambda.Code.asset("lambdas/niceOmsOrderUpdates"),
        handler: "niceOmsOrderUpdatesLambda.handler",
        timeout: cdk.Duration.minutes(15),
        logRetention: logs.RetentionDays.ONE_MONTH,
        reservedConcurrentExecutions: 1,
        memorySize: 256,
    ...
    niceOmsOrderUpdatesLambda.addEventSource(
      new cdk.aws_lambda_event_sources.SqsEventSource(niceOmsOrderUpdatesSQS, {
        batchSize: 1,
        enabled: true,
      })
    );
